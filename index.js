...
app.action({ block_id: "url_ticket" }, async ({ action, ack }) => {
  // it’s a valid email, accept the submission
  console.log("URL ticket :" + action);
  await ack();
});
app.action({ block_id: "acknowledged" }, async ({ action, ack }) => {
  // it’s a valid email, accept the submission
  console.log("Acknowledged is this : " + action);
  await ack();
});
